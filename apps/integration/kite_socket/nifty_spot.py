...
def on_ticks(ws, ticks):
    if ticks:
        ...
        cache.set("NIFTY_LTP", ticks[-1]["last_price"])
...
def nifty_spot_connect_kws():
    send_message(f"{dt.datetime.now().replace(microsecond=0)} NIFTY SPOT KWS")
